export default function SaveTime() {
    return (
        <section>
            <div className="save__container width">
                <div className="save__one">
                    <p className="sub-font">Save Time and Money</p>
                    <h2 className="main-font">Save your time and money by <br /> choosing our professional team</h2>
                </div>
                <div className="save__two"> 
                    <div className="save__two-icon">
                        <img className="savetime-icon" src="/src/assets/icon-07.png" alt="icon-07" />
                        <span>95%</span>
                        <p>Customer Satisfaction</p>
                    </div>
                    <div className="save__two-icon">
                        <img className="savetime-icon" src="/src/assets/icon-08.png" alt="icon-08" />
                        <span>80%</span>
                        <p>Increase Sales</p>
                    </div>
                </div>
            </div>
        </section>
    )
}